...
public class Joycon
{
    ...
    private void ExtractIMUValues(byte[] report_buf, int n = 0)
    {
        ...
        acc_g.X = acc_r[0] * 0.00025f;
        gyr_g.X = (gyr_r[0] - gyr_neutral[0]) * 0.00122187695f;
        if (Math.Abs(acc_g.X) > Math.Abs(max[0]))
            max[0] = acc_g.X;

        acc_g.Y = acc_r[1] * 0.00025f;
        gyr_g.X = (gyr_r[1] - gyr_neutral[1]) * 0.00122187695f;
        if (Math.Abs(acc_g.Y) > Math.Abs(max[1]))
            max[1] = acc_g.Y;

        acc_g.Z = acc_r[2] * 0.00025f;
        gyr_g.Z = (gyr_r[2] - gyr_neutral[2]) * 0.00122187695f;
        if (Math.Abs(acc_g.Z) > Math.Abs(max[2]))
            max[2] = acc_g.Z;
    }
